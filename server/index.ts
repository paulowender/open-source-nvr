...
async function jobWorker(seq: number, d: JobData): Promise<JobReturn> {
  ...
  if (d.task === JobTask.ML) {
    ...
    await new Promise((acc, _rej) => {
      ...
      const ml_task = spawn(
        cmd,
        rest.map((a) => (a === "{in}" ? input : a === "{out}" ? outpic : a)),
        { cwd: settingsCache.settings.mlDir, timeout: 120000 }
      );

      ml_task.stdout.on("data", (data: string) => {
        ml_stdout += data;
      });
      ml_task.stderr.on("data", (data: string) => {
        ml_stderr += data;
      });
      ml_task.on("error", async (error: Error) => {
        ml_error = `${error.name}: ${error.message}`;
      });
        ...
        if (ml_succcess) {
          ...
          const tags = mltags
            ? mltags.map((d) => {
                ...
                return {
                  tag: d.substr(0, i),
                  probability: parseInt(d.substr(i + 2, d.length - i - 3)),
                };
              })
            : [];
    ...
    const code = await new Promise((acc, _rej) => {
      var ffmpeg = spawn(
        ...
        [
          ...
          "0",
          "-i",
          `${c.disk}/${c.folder}/stream${m.startSegment + 1}.ts`,
          "-hide_banner",
          "-loglevel",
          "error",
          "-frames:v",
          ...
          "-q:v",
          "2",
          `${c.disk}/${c.folder}/image${d.movement_key}.jpg`,
        ],
        { timeout: 120000 }
      );
      let ff_stdout = "",
      ...
      ffmpeg.on("close", async (ff_code: number) => {
        ...
        await movementdb.put(d.movement_key, {
          ...m,
          ffmpeg: {
            success: ff_succcess,
            ...(!ff_succcess && {
...
async function processMovement(
  ...
  if (
    ...
    (movementStatus?.fail &&
      (!movementStatus.check_after || movementStatus.check_after > Date.now()))
  ) {
    return;
  }
  ...
  cameraCache[cameraKey] = {
    ...cameraCache[cameraKey],
    movementStatus: {
      ...cameraCache[cameraKey].movementStatus,
      in_progress: true,
    },
  };

  const { ip, passwd, disk, folder, secWithoutMovement, secMaxSingleMovement } =
    ce;
  try {
    const current_movement = movementStatus?.current_movement;

    const body_json = await server_fetch(
    ...
    if (body[0].error) {
      cameraCache[cameraKey] = {
        ...
        movementStatus: {
          fail: true,
          in_progress: false,
          status: `fetch movement error: ${JSON.stringify(body)}`,
      ...
      if (!current_movement) {
        // got NEW movement
        console.log(
          `processMovement: Got NEW movement (${cameraCache[cameraKey].ce.name})`
        );
        ...
        const lhs_seg_duration_seq = parseInt(
          targetduration && targetduration.length > 1 ? targetduration[1] : "2"
        );
        ...
        console.log(
          `processMovement: continuatation movement (${
            cameraCache[cameraKey].ce.name
          }) (${
            current_movement.seconds +
            1 +
        ...
        cameraCache[cameraKey] = {
          ...
          movementStatus: {
            ...
            in_progress: false,
            fail: false,
            status: "continuatation of same movment event",
            current_movement: {
      ...
      if (current_movement) {
        ...
        if (
          current_movement.consecutivesecondswithout > secWithoutMovement ||
          current_movement.seconds > (secMaxSingleMovement || 600)
        ) {
          console.log(
            `processMovement:  movement complete (${cameraCache[cameraKey].ce.name}) (${current_movement.seconds}s)`
          );

          // no movement for too long, end movement and queue Job for Image processing
          const movement_key =
            ((current_movement.startDate / 1000) | 0) - 1600000000;
          ...
          cameraCache[cameraKey] = {
            ...
            movementStatus: {
              ...
              fail: false,
              status:
                "movement ended, recorded to database key=${movement_key}",
            },
          };
        } else {
          ...
          cameraCache[cameraKey] = {
            ...
            movementStatus: {
              ...
              fail: false,
              status:
                "continuatation of same movment event (within consecutivesecondswithout)",
              current_movement: {
                ...movementStatus.current_movement,
                consecutivesecondswithout:
                  current_movement.consecutivesecondswithout + 1,
              },
            },
          };
        }
        ...
        cameraCache[cameraKey] = {
          ...
          movementStatus: {
            ...
            in_progress: false,
            fail: false,
            status: "no current movement",
          },
        };
      }
    }
  } catch (e) {
    const filtersensitive = e?.message
      ? e.message.replaceAll(passwd, "****").replaceAll(ip, "****")
      : e;
    console.warn(
      `${new Date()}: processMovement failed error: cameraKey=${cameraKey}, name=${
        cameraCache[cameraKey]?.ce?.name
      }  message=${filtersensitive}`
    );
    cameraCache[cameraKey] = {
      ...cameraCache[cameraKey],
      movementStatus: {
...
async function StreamingController(
  ...
  if (ffmpeg_process) {
    ...
    if (ffmpeg_process.check_after && ffmpeg_process.check_after < Date.now()) {
      if (ffmpeg_process.running) {
        console.log(
          `Checking ffmpeg for [${ce.name}] running=${ffmpeg_process.running} error=${ffmpeg_process.error}...`
        );
        try {
          ...
          if (last_updated_ago > 10000 /* 10 seconds */) {
            console.warn(
              `ffmpeg no output for 10secs for ${ce.name} - file ${streamFile}, kill process`
            );
            ...
            console.log(
              `Checking ffmpeg for [${ce.name}], all good, last_updated_ago=${last_updated_ago}mS, check again in 1minute`
            );
            cameraCache[cameraKey].ffmpeg_process = {
              ...(cameraCache[cameraKey].ffmpeg_process as ProcessInfo),
              check_after: Date.now() + 60000,
            };
          }
        } catch (e) {
          console.warn(
            `cannot access ffmpeg output for ${ce.name} - file ${streamFile}, kill process`
          );
          // kill, should trigger ffmpeg.on('close') thus shoud trigger check_after error nexttime around
          ffmpeg_process.taskid?.kill();
        }
        ...
        console.warn(
          `Try to restart failed or stopped ffmpeg for [${ce.name}] running=${ffmpeg_process.running} error=${ffmpeg_process.error}`
        );
      }
    } else {
  ...
  try {
    ...
    const body_json = await server_fetch(
      `http://${ce.ip}/cgi-bin/api.cgi?cmd=Login&token=null`,
      { timeout: 5000 },
      [
        {
          cmd: "Login",
          action: 0,
          param: { User: { userName: "admin", password: ce.passwd } },
        },
      ]
    );
    const body = JSON.parse(body_json);
    if (body[0].error) {
      cameraCache[cameraKey] = {
        ...
        ffmpeg_process: {
          ...
          error: true,
          in_progress: false,
          status: `Unable to retrive token: ${JSON.stringify(body[0])}`,
          check_after: Date.now() + 60000,
        },
      };
    } else {
      ...
      console.log(
        new Date().toTimeString().substring(0, 8) +
          ` starting ffmpeg for [${ce.name}] : ${streamFile}...`
      );
      var ffmpeg: ChildProcessWithoutNullStreams = spawn("/usr/bin/ffmpeg", [
        "-r",
        "25",
        "-i",
        `rtmp://admin:${ce.passwd}@${ce.ip}/bcs/channel0_main.bcs?token=${token}&channel=0&stream=0`,
        "-hide_banner",
        "-loglevel",
        "error",
        "-vcodec",
        "copy",
        "-start_number",
        (((Date.now() / 1000) | 0) - 1600000000).toString(),
        streamFile,
      ]);
      cameraCache[cameraKey] = {
        ...cameraCache[cameraKey],
        ffmpeg_process: {
          ...
          status: "Starting...\n",
          check_after:
            Date.now() + 60000 /* check it hasn't hung every 30seconds */,
        },
      };
      ...
      ffmpeg.stdout.on("data", (data: string) => {
        ...
        cameraCache[cameraKey] = {
          ...
          ffmpeg_process: {
            ...cameraCache[cameraKey].ffmpeg_process,
            status:
              cameraCache[cameraKey].ffmpeg_process?.status + data.toString(),
          } as ProcessInfo,
        };
      });
      ffmpeg.stderr.on("data", (data: string) => {
        ...
        cameraCache[cameraKey] = {
          ...
          ffmpeg_process: {
            ...cameraCache[cameraKey].ffmpeg_process,
            status:
              cameraCache[cameraKey].ffmpeg_process?.status + `StdErr: ${data}`,
          } as ProcessInfo,
        };
      });
      ffmpeg.on("error", async (error: Error) => {
        console.warn(
          `ffmpeg on-error [${ce.name}]: ${error.name}: ${error.message}`
        );
        cameraCache[cameraKey] = {
          ...cameraCache[cameraKey],
          ffmpeg_process: {
            ...cameraCache[cameraKey].ffmpeg_process,
            status:
              cameraCache[cameraKey].ffmpeg_process?.status +
      ...
      ffmpeg.on("close", async (code: number) => {
        console.warn(
          new Date().toTimeString().substring(0, 8) +
            ` ffmpeg on-close [${ce.name}]: code=${code}`
        );
        cameraCache[cameraKey] = {
          ...cameraCache[cameraKey],
          ffmpeg_process: {
    ...
    console.warn(
      new Date().toTimeString().substring(0, 8) +
        ` FFMpeg catch error [${ce.name}]: ${e}, try again in 1minute`
    );
    cameraCache[cameraKey] = {
      ...cameraCache[cameraKey],
      ffmpeg_process: {
  ...
  var assets = new Router()
    .get("/image/:moment", async (ctx, _next) => {
      ...
      try {
        ...
        const serve = `${c.disk}/${c.folder}/${
          m.ml && m.ml.success ? "mlimage" : "image"
        }${moment}.jpg`;
        const { size } = await fs.stat(serve);
        ctx.set("content-type", "image/jpeg");
        ctx.body = createReadStream(serve, { encoding: undefined }).on(
          "error",
          ctx.onerror
        );
      } catch (e) {
    ...
    .get(
      "/video/:startSegment/:seconds/:cameraKey/:file",
      async (ctx, _next) => {
        ...
        if (file.endsWith(".m3u8")) {
          ...
          if (isNaN(segmentInt) || isNaN(secondsInt)) {
            ctx.throw(
              400,
              `message=${startSegment} or ${seconds} not valid values`
            );
          } else {
            //const startSegment = segment // (d / 1000 | 0) - 1600000000
            const preseq: number = ctx.query["preseq"]
              ? parseInt(ctx.query["preseq"] as any)
              : 0;
            const postseq: number = ctx.query["postseq"]
              ? parseInt(ctx.query["postseq"] as any)
              : 0;
...
` +
              [...Array(Math.round(secondsInt / 2) + preseq + postseq).keys()]
                .map(
                  (n) => `#EXTINF:2.000000,
stream${n + segmentInt - preseq}.ts`
                )
                .join("\n") +
              "\n" +
              "#EXT-X-ENDLIST\n";
...
${ce.name}.${n + m.startSegment - preseq}.ts`).join("\n") + "\n" + "#EXT-X-ENDLIST\n"
        ...
        await new Promise(async (res, rej) => {
          const mv_task = spawn(
            ...
            [
              "-y",
              "-i",
              `http://localhost:${PORT}/video/${startSegment}/${seconds}/${cameraKey}/stream.m3u8${
                preseq > 0 || postseq > 0
                  ? `?preseq=${preseq}&postseq=${postseq}`
                  : ""
              }`,
              "-c",
              "copy",
          ...
          mv_task.on("error", async (error: Error) => {
            myerror = `${error.name}: ${error.message}`;
          });
    ...
    .get(["/(.*)"], async (ctx, _next) => {
      ...
      await send(ctx, !path || path === "video_only" ? "/index.html" : path, {
        root: process.env["WEBPATH"] || "./build",
      });
    });
  ...
  const api = new Router({ prefix: "/api" })
    .post("/settings", async (ctx, _next) => {
      ...
      if (ctx.request.body) {
        ...
        try {
          ...
          if (!dirchk.isDirectory())
            throw new Error(`${new_settings.disk_base_dir} is not a directory`);
          ...
          settingsCache = {
            ...
            status: {
              ...
              nextCheckInMinutes: new_settings.enable_cleanup
                ? new_settings.cleanup_interval
                : 0,
            },
          };
          ctx.status = 201;
    ...
    .post("/camera/:id", async (ctx, _next) => {
      ...
      console.log(
        `camera save ${cameraKey} -  ${JSON.stringify(ctx.request.body)}`
      );
      if (ctx.request.body) {
    ...
    .get("/movements", async (ctx, _next) => {
      ...
      const cameras: CameraEntryClient[] = Object.keys(cameraCache)
        .filter((c) => !cameraCache[c].ce.delete)
        .map((key) => {
          ...
          return {
            key,
            ...cameraEntry,
            ffmpeg_process: ffmpeg_process_wo_task,
            movementStatus,
          } as CameraEntryClient;
        });
      ...
      ctx.body = await new Promise(async (res, _rej) => {
        ...
        if (mode === "Time") {
          ...
          const feed = movementdb
            .createReadStream({
              reverse:
                true /*, gt: oldestctimeMs > 0 ? (oldestctimeMs / 1000 | 0) - 1600000000 : 0 */,
            })
            .on("data", (data) => {
              ...
              if (mode === "Filtered") {
                if (tags && Array.isArray(tags) && tags.length > 0) {
                  ...
                  if (
                    ignore_tags &&
                    Array.isArray(ignore_tags) &&
                    ignore_tags.length > 0
                  ) {
                    tags = tags.reduce(
                      (a, c) => (ignore_tags.includes(c.tag) ? a : a.concat(c)),
                      []
                    );
                  }
                }
                ...
                movements.push({
                  ...
                  startDate_en_GB: new Intl.DateTimeFormat("en-GB", {
                    ...(startDate.toDateString() !==
                      new Date().toDateString() && { weekday: "short" }),
                    minute: "2-digit",
                    hour: "2-digit",
                    hour12: true,
                  }).format(startDate),
  ...
  const nav = new Router()
    .get("/network", async (ctx, _next) => {
      ctx.redirect(
        `http://${
          ctx.headers.host ? ctx.headers.host.split(":")[0] : "localhost"
        }:3998`
      );
    })
    .get("/metrics", async (ctx, _next) => {
      ctx.redirect(
        `http://${
          ctx.headers.host ? ctx.headers.host.split(":")[0] : "localhost"
        }:3000/d/T3OrKihMk/our-house?orgId=1`
      );
    });
...
async function clearDownDisk(
  diskDir: string,
  cameraKeys: Array<string>,
  cleanupCapacity: number
): Promise<DiskCheckReturn> {
  const diskres = await diskCheck(
    diskDir,
    cameraKeys.map((key) => `${diskDir}/${cameraCache[key].ce.folder}`),
    cleanupCapacity
  );
  console.log(diskres);
  if (diskres.revmovedMBTotal > 0) {
    const mostRecentctimMs = Object.keys(diskres.folderStats).reduce(
      (acc, cur) =>
        diskres.folderStats[cur].lastRemovedctimeMs
          ? diskres.folderStats[cur].lastRemovedctimeMs > acc
            ? diskres.folderStats[cur].lastRemovedctimeMs
            : acc
          : acc,
      0
    ...
    if (mostRecentctimMs > 0 || cleanupCapacity === -1) {
      const keytoDeleteTo =
        cleanupCapacity === -1
          ? null
          : ((mostRecentctimMs / 1000) | 0) - 1600000000;
      const deleteKeys: Array<number> = await new Promise((res, _rej) => {
        let keys: Array<number> = [];
        movementdb
          .createReadStream(keytoDeleteTo && { lte: keytoDeleteTo })
          .on("data", (data) => {
            const { key, value } = data as {
              key: number;
              value: MovementEntry;
...
async function main() {
  ...
  try {
    settingsCache = {
      ...settingsCache,
      settings: (await db.get("settings")) as Settings,
    };
  } catch (e) {
  ...
  setInterval(async () => {
    ...
    if (status.nextCheckInMinutes === 0) {
      settingsCache = {
        ...settingsCache,
        status: { ...status, nextCheckInMinutes: settings.cleanup_interval },
      };
      if (settings.enable_cleanup && settings.disk_base_dir) {
        try {
          const diskres = await clearDownDisk(
            settings.disk_base_dir,
            Object.keys(cameraCache).filter(
              (c) =>
                !cameraCache[c].ce.delete && cameraCache[c].ce.enable_streaming
            ),
            settings.cleanup_capacity
          );
          settingsCache = {
            ...settingsCache,
            status: {
      ...
      settingsCache = {
        ...
        status: {
          ...status,
          nextCheckInMinutes: status.nextCheckInMinutes - 1,
        },
      };
    }
  }, 60000);
