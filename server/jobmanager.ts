...
export class JobManager {
  ...
  constructor(
    db /*: LevelUp*/,
    concurrency: number,
    workerfn: (seq: number, d: JobData) => Promise<JobReturn>
  ) {
    this._db = db;
  ...
  async start(continueRun: boolean = false) {
    ...
    if (!continueRun) {
      ...
      const { nextSequence, numberCompleted, numberRunning, nextToRun } =
        await new Promise((res, _rej) => {
          this._control.get(0, async (err: any, value) => {
            if (err) {
              if (err.notFound) {
                console.error(
                  `Cannot continue, no previous run found (make sure you are in the correct directory)`
                );
                process.exit();
              }
            }
      ...
      console.log(
        `JobManager: Re-starting: queued=${nextSequence} running=${numberRunning} completed=${numberCompleted} nextToRun=${nextToRun}`
      );
      ...
      if (nextToRun > 0) {
        const rkeys: number[] = await new Promise((res, _rej) => {
          ...
          const feed = this._queue
            .createKeyStream({ lt: nextToRun })
            .on("data", (d) => {
              runningKeys.push(d);
            })
            .on("end", () => {
        ...
        if (rkeys.length > 0) {
          ...
          for (let key of rkeys) {
            await this.runit(
              key,
              queueJobData.fromBuffer(
                await this._queue.get(key)
              ) /*, runningJobData.fromBuffer(await this._running.get(key))*/
            );
          }
        }
      }
    ...
    this._finishedPromise = new Promise((resolve) => {
      const interval = setInterval(async () => {
        const { nextSequence, nextToRun, numberCompleted, numberRunning } =
          await this._control.get(0);
  ...
  private async checkRun(newData?: JobData | null, completed?: JobReturn) {
    ...
    let release = await this._mutex.aquire();
    let { nextSequence, nextToRun, numberCompleted, numberRunning } =
      await this._control.get(0);
    ...
    if (completed) {
      ...
      if (completed.newJob) {
        await this._queue.put(
          nextSequence++,
          queueJobData.toBuffer(completed.newJob)
        );
      }
    }
    ...
    while (
      nextToRun < nextSequence &&
      nextToRun - numberCompleted < this._limit
    ) {
      ...
      this.runit(
        ...
        newWorkNext
          ? newData
          : queueJobData.fromBuffer(
              await this._queue.get(nextToRun)
            ) /*, runningData*/
      );
      nextToRun++;
    }
  ...
  private runit(seq: number, data: JobData /*, runningData: JobRunningData*/) {
    this._workerFn(seq, data /*, runningData*/).then(
      async (value) => {
